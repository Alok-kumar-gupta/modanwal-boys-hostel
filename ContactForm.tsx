...
export default function ContactForm({ preConfig, onClearPreConfig, config, onAddBooking }: ContactFormProps) {
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const waUrl = getWhatsAppLinkForInquiry(formattedInquiry);
    try {
    ...
    <section id="contact" className="py-24 bg-white relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start" id="form-faq-grid">
            ...
            <AnimatePresence mode="wait">
              {!isSubmitted ? (
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      id="notes"
                      rows={3}
                      placeholder="Write your message or specific queries here..."
                      value={notes}
                ...
                >
                  ...
                  {/* Direct WhatsApp notification status */}
                  <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 text-emerald-950 flex flex-col sm:flex-row items-center justify-between gap-3 text-left">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-bold text-xs sm:text-sm text-emerald-950">
                          Direct WhatsApp Alert Sent to Caretaker!
                        </p>
                        <p className="text-[11px] text-emerald-700">
                          (सीधे केयरटेकर को व्हाट्सएप पर नोटिफिकेशन भेजा गया है)
                        </p>
                      </div>
                    </div>
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs px-4 py-2 rounded-xl flex items-center justify-center gap-1.5 transition-all shrink-0 cursor-pointer shadow-xs"
                      id="direct-whatsapp-inquiry-btn"
                    >
          ...
          {/* Hostel Quick Contact & Payment Panel (Right Column) */}
          <div className="lg:col-span-5 space-y-6" id="contact-info-card">
            
            {/* Direct Information Card */}
            <div className="bg-slate-50/70 border border-slate-200 rounded-3xl p-6 sm:p-7 space-y-5">
              <div className="border-b border-slate-200/80 pb-4">
                <h3 className="font-display font-bold text-xl text-slate-900">Hostel Address & Direct Contacts</h3>
                <p className="text-xs text-slate-500 mt-1">Visit us directly or contact the caretaker</p>
              </div>

              <div className="space-y-4 text-xs">
                <div className="flex items-start gap-3">
                  <div className="p-2 bg-white rounded-xl border border-slate-200 text-slate-800 shrink-0">
                    <MapPin className="w-4 h-4 text-primary-600" />
                  </div>
                  <div>
                    <span className="font-bold text-slate-900 block">Hostel Address:</span>
                    <span className="text-slate-600 leading-relaxed block">{config.address || 'Tindola, Near SRMU Gate, Barabanki, Uttar Pradesh - 225003'}</span>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <span className="font-bold text-slate-900 block">Caretaker Helpline:</span>
                    <a href={`tel:${config.phone.replace(/[^0-9]/g, '')}`} className="text-emerald-700 font-bold hover:underline block">
                      {config.phone} ({config.caretakerName || 'Alok Caretaker'})
                    </a>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <a href={`mailto:${config.email}`} className="text-indigo-700 font-medium hover:underline block">
                      {config.email}
                    </a>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-slate-600 block">9:00 AM – 7:00 PM (Daily for Parents/Visitors)</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="mt-12" id="contact-map-wrapper">
          <HostelMap />
        </div>
      </div>
    </section>
