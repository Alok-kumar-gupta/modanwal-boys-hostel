...
const SEED_MAINTENANCE_LOGS: MaintenanceLog[] = [
  {
    ...
    roomNumber: 'Room 104',
    issueType: 'plumbing',
    description: 'Bathroom tap leaking continuously, needs washer replacement.',
    severity: 'medium',
    status: 'pending',
    reportedDate: '2026-07-15',
    assignedTo: 'Ramesh Plumber',
    notes: 'Informed Ramesh. He will visit on Monday morning.'
  },
  {
    id: 'm-2',
    roomNumber: 'Mess Kitchen',
    issueType: 'appliance',
    description: 'Commercial mixer grinder making grinding noise and overheating.',
    severity: 'high',
    status: 'in-progress',
    reportedDate: '2026-07-17',
    assignedTo: 'Sujata Service Center',
    cost: 1200,
    notes: 'Sent to service center. Parts being replaced.'
  },
  {
    id: 'm-3',
    roomNumber: 'Room 208',
    issueType: 'electrical',
    description: 'Ceiling fan capacitor dead, running very slow.',
    severity: 'low',
    status: 'resolved',
    reportedDate: '2026-07-10',
    resolvedDate: '2026-07-11',
    assignedTo: 'Local electrician (Sunil)',
    cost: 350,
    notes: 'Capacitor replaced with Orient 2.5mfd. Working perfectly fine now.'
  },
  {
    id: 'm-4',
    roomNumber: 'First Floor Corridor',
    issueType: 'internet',
    description: 'Wi-Fi Extender status LED red, poor speed on first floor.',
    severity: 'high',
    status: 'resolved',
    reportedDate: '2026-07-12',
    resolvedDate: '2026-07-12',
    assignedTo: 'ACT Fibernet Support',
    notes: 'Power cycled and re-configured. Signal strength back to full.'
  }
];
...
export default function MaintenanceTab() {
  ...
  return (
    <div className="space-y-6" id="maintenance-dashboard-root">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4" id="maintenance-stats-container">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm flex items-center gap-3">
          ...
          <div>
            <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Resolved (हल किया गया)</span>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm flex items-center gap-3">
          ...
          <div>
            <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Total Expenses (कुल खर्च)</span>
      ...
      <AnimatePresence>
        {isAddFormOpen && (
          ...
          >
            <form onSubmit={handleCreateLog} className="bg-slate-900 text-white p-5 rounded-2xl border border-slate-800 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-1">
                  <label className="block text-[10px] font-bold text-slate-400 uppercase">Room or Location (कमरा / स्थान) *</label>
                  <input
                    type="text"
                    required
                    placeholder="e.g. Room 102, Kitchen, Lobby"
                    value={newRoomNumber}
                ...
                {/* Issue Type */}
                <div className="space-y-1">
                  <label className="block text-[10px] font-bold text-slate-400 uppercase">Category (श्रेणी) *</label>
                  <select
                    value={newIssueType}
                ...
                {/* Severity */}
                <div className="space-y-1">
                  <label className="block text-[10px] font-bold text-slate-400 uppercase">Severity (गंभीरता) *</label>
                  <select
                    value={newSeverity}
                    onChange={(e) => setNewSeverity(e.target.value as any)}
                    className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3.5 py-2 text-xs focus:ring-1 focus:ring-amber-500 focus:border-amber-500 outline-none text-white cursor-pointer"
                  >
                    <option value="low">Low Priority (कम प्राथमिकता)</option>
              ...
              {/* Description */}
              <div className="space-y-1">
                <label className="block text-[10px] font-bold text-slate-400 uppercase">Issue Description (समस्या का विवरण) *</label>
                <textarea
                  required
                  rows={2}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Assigned To */}
                <div className="space-y-1">
                  <label className="block text-[10px] font-bold text-slate-400 uppercase">Assigned Technician (तकनीशियन का नाम)</label>
                  <input
                    type="text"
                    placeholder="e.g. Electrician Sunil, Caretaker Modanwal"
                    value={newAssignedTo}
                ...
                {/* Estimated Cost */}
                <div className="space-y-1">
                  <label className="block text-[10px] font-bold text-slate-400 uppercase">Estimated/Incurred Cost (अनुमानित खर्च, ₹)</label>
                  <input
                    type="number"
                    placeholder="e.g. 500"
                    value={newCost}
              ...
              {/* Resolution Notes */}
              <div className="space-y-1">
                <label className="block text-[10px] font-bold text-slate-400 uppercase">Internal Notes (आंतरिक नोट)</label>
                <input
                  type="text"
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm space-y-3" id="maintenance-filters-panel">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <div className="relative col-span-1 md:col-span-1">
            ...
            <input
              type="text"
              placeholder="Search room, desc, technician..."
              value={searchQuery}
          ...
          <div>
            <select
              ...
              className="w-full border border-slate-200 rounded-xl px-3 py-2 text-xs focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-slate-700 bg-white cursor-pointer"
            >
              <option value="all">All Priorities (सभी प्राथमिकताएं)</option>
                    ...
                    <tr key={log.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="p-4 space-y-1 max-w-xs">
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-slate-400">
                          ...
                          {log.resolvedDate && (
                            <span className="text-emerald-600 flex items-center gap-0.5 bg-emerald-50 px-1.5 py-0.5 rounded-md">
                              Resolved: {log.resolvedDate}
                            </span>
                          )}
                        </div>
                        {log.notes && (
                          <p className="text-[10px] text-slate-500 italic bg-slate-50 p-1.5 rounded-lg border border-slate-100 mt-1">
              ...
              {filteredLogs.map((log) => (
                <div key={log.id} className="p-4 space-y-3 hover:bg-slate-50/50 transition-colors text-slate-800">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <span className="block font-display font-extrabold text-sm text-slate-900">{log.roomNumber}</span>
            ...
            >
              ...
              <form onSubmit={handleSaveEdit} className="p-5 space-y-4 text-left overflow-y-auto flex-1 scrollbar-none">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="block text-[9px] font-bold text-slate-400 uppercase">Room or Location *</label>
                    <input
                      type="text"
                      required
